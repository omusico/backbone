...
- (void)drawBarChart:(NSDictionary*)dataDict {
  NSArray* dataPlots = dataDict[@"data"];
  UIColor* barColor1 = ( dataDict[@"color1"] != nil ) ? [RCTConvert UIColor:dataDict[@"color1"]] : self.parentChartView.defaultColor;
  UIColor* barColor2 = ( dataDict[@"color2"] != nil ) ? [RCTConvert UIColor:dataDict[@"color2"]] : self.parentChartView.defaultColor;
  ...
  CGFloat barWidth = (axisWidth / self.parentChartView.xLabels.count * [self.parentChartView horizontalScale] * 0.5) * widthPercent / 2; // 60% of the full width
  
  for ( NSUInteger i = 0; i < dataPlots.count; i += 2) {
    
    CGFloat s = axisHeight / (maxBound - minBound);
    CGPoint point = [self getPointForIndex:i data:dataPlots withScale:s];
    CGPoint point2 = [self getPointForIndex:i+1 data:dataPlots withScale:s];
    point.y +=  minBound * s;
    point2.y += minBound * s;
    
    CGFloat height = axisHeight - point.y;
    CGFloat height2 = axisHeight - point2.y;
    UIBezierPath* circle = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(point.x - barWidth * 2, point.y, barWidth * 1.25, height) cornerRadius:1.0];
    UIBezierPath* circle2 = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(point.x - barWidth / 50, point2.y, barWidth * 1.25, height2) cornerRadius:1.0];
    ...
    fillLayer.strokeColor = barColor1.CGColor;
    fillLayer.fillColor = barColor1.CGColor;
    fillLayer.lineWidth = 1;
    fillLayer.lineJoin = kCALineJoinRound;
    [self.layer addSublayer:fillLayer];
    [self.layers addObject:fillLayer];

    CAShapeLayer *fillLayer2 = [CAShapeLayer layer];
    fillLayer2.frame = CGRectMake(point2.x, point2.y, barWidth, height2);
    fillLayer2.bounds = CGRectMake(point2.x, point2.y, barWidth, height2);
    fillLayer2.path = circle2.CGPath;
    fillLayer2.strokeColor = barColor2.CGColor;
    fillLayer2.fillColor = barColor2.CGColor;
    fillLayer2.lineWidth = 1;
    fillLayer2.lineJoin = kCALineJoinRound;
    [self.layer addSublayer:fillLayer2];
    [self.layers addObject:fillLayer2];
